...
static GDALDataset *buildMLXCOG(GDALDataset *poSrcDS, const char *outPath)
{
    ...
    // Allocate overview structure via NEAREST (fast CPU)
    CPLErr eErr = poTmpDS->BuildOverviews(
        "NEAREST", static_cast<int>(ovrLevels.size()), ovrLevels.data(),
...
int main()
{
    ...
    GDALDataset *poMLX  = buildMLXCOG(poSrcDS, MLX_OUT);
    ...
    for (int iBand = 1; iBand <= nBands; iBand++)
    {
        ...
        {
            GDALRasterBand *gdalOvr = poGDAL->GetRasterBand(iBand)->GetOverview(iOvr);
            GDALRasterBand *mlxOvr  = poMLX->GetRasterBand(iBand)->GetOverview(iOvr);
